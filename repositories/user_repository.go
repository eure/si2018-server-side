...
func (r *UserRepository) FindWithCondition(limit, offset int, gender string, ids []int64) ([]entities.User, error) {
	...
	err := s.Table("user").Select("user.* , path").Join("INNER", "user_image", "user.id = user_image.user_id").Find(&users)
	if err != nil {
		return users, err
	}
...
func (r *UserRepository) FindByIDs(ids []int64) ([]entities.User, error) {
	...
	err := engine.Table("user").Select("user.* , path").Join("INNER", "user_image", "user.id = user_image.user_id").In("id", ids).Find(&users)
	if err != nil {
		return users, err
	}
